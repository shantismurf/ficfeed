...
import { DiscordClient, formattedDate } from './utilities.js';
const client = DiscordClient.getInstance();
import config from './config.json' with {type: 'json'};

const test = false; //set in ao3api.js and index.js
...
async function fetchDataWithHeaders(url) {
    ...
    let retryMessage = await feedChannel.send(`Please wait. Processing <${url}>`); // Initial message
    ...
    while (retryCount < maxRetries) {
        try {
            ...
            // Update the retry message instead of sending a new one
            if (error.response && error.response.headers['retry-after']) {
                ...
                await retryMessage.edit(`Please wait. Processing <${url}> \nAO3 is rate limiting. Retrying in ${retryAfter} seconds...`);
                await new Promise(resolve => setTimeout(resolve, retryAfter * 1000));
            } else {
                console.log(`***Retry at ${formattedDate()}:index: ${retryCount}, delay: ${delay}ms, maxRetries: ${maxRetries}\n${error}`);
                await retryMessage.edit(`Please wait. Processing <${url}>\nRetry attempt ${retryCount + 1}, delay ${delay}ms`);
                await new Promise(resolve => setTimeout(resolve, delay)); // Default to 1 second delay
            }
            delay = String(error).includes('525') ? delay * 2 : delay;
            maxRetries = String(error).includes('525') ?  maxRetries + 1 : maxRetries;
            retryCount++;
            if (maxRetries > 10) { retryCount = maxRetries; } // Prevents infinite loop
            if (retryCount === maxRetries) {
                console.error(`*!*!*!Max retries exceeded at ${formattedDate()}:${error}`);
                await retryMessage.edit(`Archive is unavailable. Max retries exceeded.\n${error}`);
...
export default async function ao3api(link) {
    let metadata = {};
    ...
    try {
        ...
        if (link.includes("works")) {
            ...
            if (metadata.workTitle == '') {
                console.log(`*!*!*!Work title not found at ${formattedDate()}`);
            ...
            if (metadata.seriesTitle == '') {
                console.log(`*!*!*!Series title not found at ${formattedDate()}`);
                return { metadata, error: true }
            };
            ...
            if (metadata.collectionTitle == '') {
                console.log(`*!*!*!Collection title not found at ${formattedDate()}`);
                return { metadata, error: true }
            };
        ...
        console.log(`*!*!*!Link ${link} failed to be processed at ${formattedDate()}.`);
        console.log(e)
